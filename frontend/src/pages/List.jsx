...
export default function List() {
  ...
  useEffect(() => {
    dispatch(fetchEvents({ status: status || undefined, severity: severity || undefined, event_type: type || undefined }));
  ...
  return (
    <div className="container">
      <div className="toolbar">
        ...
        <Link to="/wizard" className="primary">+ New Event</Link>
      </div>
        ...
        <div className="row">
          ...
          <label>Type
            <select value={type} onChange={(e) => setType(e.target.value)}>
              {TYPES.map(s => <option key={s} value={s}>{s || "All"}</option>)}
            </select>
          </label>
        </div>
      </div>
          ...
          <div className="tableWrap">
            <table>
              ...
              <tbody>
                ...
                {!loading && sorted.length === 0 && <tr><td colSpan="7">No events</td></tr>}
                {!loading && sorted.map(e => (
                  <tr key={e.id}>
                    ...
                    <td>{e.event_type}</td>
                    <td>{e.status}</td>
                    <td>{e.due_date ? dayjs(e.due_date).format("YYYY-MM-DD HH:mm") : "-"}</td>
                    <td>{e.initiator}</td>
                    <td className={`sev ${e.severity.toLowerCase()}`}>{e.severity}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card">
          <div className="aiHeader">
            ...
            <select value={selectedId} onChange={(e) => setSelectedId(e.target.value)}>
              ...
              {items.map(e => <option key={e.id} value={e.id}>#{e.id} — {e.title}</option>)}
            </select>
          </div>

          <div className="aiButtons">
            <button onClick={() => runAI("high_risk")}>Show high-risk events</button>
            <button onClick={() => runAI("summarize_open_last_month")}>Summarize open events (30d)</button>
            <button onClick={() => runAI("suggest_next_steps")}>Suggest next steps (selected)</button>
            <button onClick={() => runAI("capa_trends")}>Identify CAPA trends</button>
            <button onClick={() => runAI("closure_draft")}>Generate closure draft (selected)</button>
            <button className="ghost" onClick={() => dispatch(clearAI())}>Clear</button>
          </div>
            ...
            {aiResult?.data && (
              <>
                ...
                {"gemini_text" in aiResult.data
                  ...
                  : <pre className="prewrap">{JSON.stringify(aiResult.data, null, 2)}</pre>}
              </>
            )}
          </div>
        </div>
